#!/usr/bin/env python3
"""
Pick-and-Place Controller for Perfect Size V3
...
Key features:
  ...
  - Uses grasp_detector node for contact-based attach/detach (V3)
  - Synchronized gripper finger movement with mimic joints (V3)
  - Adjusts pick/place heights based on cube size

Author: Perfect Size V3 Architecture
"""
...
class PickPlaceController(Node):
    ...
    def __init__(self):
        ...
        # V3: Use grasp_detector for attach/detach instead of direct DetachableJoint calls
        # Publisher to tell grasp_detector which cube to attach when gripper closes
        self._pending_grasp_pub = self.create_publisher(
            String,
            '/gripper/pending_grasp',
            10
        )
        
        # DetachableJoint publishers for initial detach only
        # Small cubes (6cm)
        ...
        # Attach publishers still needed for grasp_detector communication
        ...
        self.get_logger().info('Pick-Place Controller (Size V3 - Contact-Based Grasping) initializing...')
    ...
    def _signal_pending_grasp(self, cube: CubeTarget):
        """
        V3: Signal to grasp_detector which cube should be attached when gripper closes.
        The grasp_detector monitors gripper position and triggers attach automatically.
        """
        cube_name = cube.name if cube.name else f'{cube.cube_key}_cube'
        label = f'{cube.size} {cube.color}'
        self.get_logger().info(f'[GRASP] Signaling pending grasp: {label}')
        
        # Publish the cube name so grasp_detector knows what to attach
        msg = String()
        msg.data = cube_name
        self._pending_grasp_pub.publish(msg)
        
        # Also directly trigger attach for reliability (grasp_detector may not be running)
        # This ensures backward compatibility
        self._attach_cube_direct(cube)
    
    def _attach_cube_direct(self, cube: CubeTarget):
        """Direct attach using DetachableJoint (fallback/reliability)."""
        msg = Empty()
    ...
    def pick_and_place_cube(self, cube: CubeTarget):
        ...
        # Grasp cube - V3: Signal which cube to attach, then close gripper
        # The grasp_detector node will handle the actual attach when it detects
        # the gripper has closed
        self.get_logger().info(f'\n── Grasp {label} Cube ──')
        self._signal_pending_grasp(cube)
        self.close_gripper()
        time.sleep(0.3)  # Wait for contact-based attachment
        ...
        # Release cube - Open gripper and explicitly detach
        self.get_logger().info(f'\n── Release {label} Cube ──')
        self.detach_cube(cube)  # Explicit detach for reliable release
        self.open_gripper()
        time.sleep(0.3)
    ...
    def run(self):
        ...
        self.get_logger().info('   Pick-and-Place Controller - Size V3')
        self.get_logger().info('   Realistic Gripper • Contact-Based Grasping')
